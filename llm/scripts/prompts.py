...
2. 每个概念词一行
3. 如果找不到概念词，返回"无"
4. 返回出现频率最高的概念词（至少6个），这些文本是围绕这个词所重点讨论的，你不可以随便找几个词敷衍。
...
4. **内容来源**：尽量把输入列表中提供的原文内容（且你判定为相关句子）串联起来，放置到HTML的合适位置。
5. **串联逻辑**：并且用你自己的知识库，来把这些原文内容串联，在内容和逻辑上完整，而且概念的定义要清晰。特别如果这是一个领域内的通用概念。
5. **不要发散**：你增加的内容是为了提高可读性，让读者容易理解，而不是发散太多内容。所以你添加的内容占比要少于50%。
...
GET_CONCEPT_DEFINITION_HTML_PROMPT_TEMPLATE_BACKUP = """请根据以下文本，为"{{concept}}"这个概念生成一个 HTML 格式的详细解释。
...
# MarkNotes 生成相关 Prompt 模板

GENERATE_MARKNOTE_HTML_PROMPT_TEMPLATE = """请根据以下点评内容（reviewContent），生成一个 HTML 格式的整理和总结。

要求：
1. 保持原意，但可以适当整理和结构化
2. 使用标准的 HTML 标签，如 <h3>, <h4>, <p>, <strong>, <em> 等
3. 需要有标题来组织内容
4. 中英文混合解释，每一句中文后面要跟着对应的英文翻译，包括标题也需要有英文翻译
5. 内容要有层次结构，使用 h3-h6 标签来组织
6. 排版美观，可读性高
7. 只返回 HTML 内容，不要有其他多余的内容（如 markdown 代码块标记、引号等）
8. 如果引用原文，使用 <strong><em>原文内容</em></strong> 标记
9. 有阿拉伯数字序号的行（例如1. 2. 3.），需要使用 h3标签
10. 学生问的问题，需要使用 h4标签
