...
#
# Guidebook 自动化流程脚本
# 执行：fetch (wereader/fetch.py 完整流程) -> generate guidebook -> import to anki
#
# 用法：
#   python guidebook_pipeline.py                    # 处理所有书籍
#   python guidebook_pipeline.py --book-id 3300089819
#   python guidebook_pipeline.py --book-name "极简央行课"
#
...
class GuidebookPipeline:
    ...
    def __init__(self):
        ...
        self.llm_script = self.project_root / "llm" / "scripts" / "generate_guidebook.py"
        self.anki_script = self.project_root / "anki" / "scripts" / "import_guidebook_to_anki.py"
    ...
    def check_prerequisites(self):
        ...
        if not self.llm_script.exists():
            print_error(f"Guidebook 生成脚本不存在: {self.llm_script}")
            sys.exit(1)
        
        if not self.anki_script.exists():
            print_error(f"Anki 导入脚本不存在: {self.anki_script}")
            sys.exit(1)
    ...
    def run_generate_guidebook(self, book_id: Optional[str] = None, book_name: Optional[str] = None) -> bool:
        """执行 generate guidebook 步骤"""
        print_step("步骤 2: 生成 Guidebook")
        
        args = [sys.executable, str(self.llm_script)]
        if book_id:
            args.extend(['--book-id', book_id])
            print_info(f"处理书籍 ID: {book_id}")
        elif book_name:
        ...
        try:
            ...
            if result.returncode == 0:
                print_success("Guidebook 生成完成")
                return True
            else:
                print_error("Guidebook 生成失败")
                return False
        except Exception as e:
            print_error(f"Guidebook 生成执行出错: {e}")
            return False
    
    def run_import_anki(self, book_id: Optional[str] = None, book_name: Optional[str] = None) -> bool:
        """执行 import to anki 步骤"""
        print_step("步骤 3: 导入到 Anki")
        
        args = [sys.executable, str(self.anki_script)]
        if book_id:
            args.extend(['--book-id', book_id])
            print_info(f"处理书籍 ID: {book_id}")
        elif book_name:
        ...
        try:
            ...
            if result.returncode == 0:
                print_success("Anki 导入完成")
                return True
            else:
                print_error("Anki 导入失败")
                return False
        except Exception as e:
            print_error(f"Anki 导入执行出错: {e}")
            return False
    ...
    def process_single_book(self, book_id: Optional[str] = None, book_name: Optional[str] = None) -> bool:
        ...
        # Step 2: Generate Guidebook
        if not self.run_generate_guidebook(book_id, book_name):
            print_error("Guidebook 生成失败，停止流程")
            return False
        
        # Step 3: Import to Anki
        if not self.run_import_anki(book_id, book_name):
            print_error("Anki 导入失败")
            return False
    ...
    def process_all_books(self):
        ...
        # Step 2 & 3: 逐本生成 guidebook 并导入 Anki
        success_count = 0
        fail_count = 0
        ...
        for bid in book_ids:
            ...
            # Step 2: Generate Guidebook
            if not self.run_generate_guidebook(bid, None):
                print_warning(f"书籍 {bid} 的 Guidebook 生成失败，跳过 Anki 导入")
                fail_count += 1
                continue
            
            # Step 3: Import to Anki
            if not self.run_import_anki(bid, None):
                print_warning(f"书籍 {bid} 的 Anki 导入失败")
                fail_count += 1
            else:
                success_count += 1
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Guidebook 自动化流程：fetch -> generate guidebook -> import to anki',
